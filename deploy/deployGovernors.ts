...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  const deploymentFee = await deployer.estimateDeployFee(artifact, [deployedContracts.votingEscrow]);
  const parsedFee = ethers.utils.formatEther(deploymentFee.toString());
  const governorContract = await deployer.deploy(artifact, [deployedContracts.votingEscrow]);
    ...
    const verificationId= await hre.run("verify:verify", {
        ...
        constructorArguments: [deployedContracts.votingEscrow],
    ...
    const deploymentFee2 = await deployer.estimateDeployFee(artifact2, [deployedContracts.forwarder,deployedContracts.votingEscrow,deployedContracts.minter]);
    const parsedFee2 = ethers.utils.formatEther(deploymentFee2.toString());
    const epochGovernorContract = await deployer.deploy(artifact2, [deployedContracts.forwarder,deployedContracts.votingEscrow,deployedContracts.minter]);
    ...
    const verificationId2= await hre.run("verify:verify", {
        ...
        constructorArguments: [deployedContracts.forwarder,deployedContracts.votingEscrow,deployedContracts.minter],
